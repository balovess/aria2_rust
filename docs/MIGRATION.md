...
完成校验，以及 SFTP 已有完整文件和传输完成后的磁盘校验，统一接入
`checksum::check_integrity::man::enqueue_file_checksum_for_group`。新增的
`FileChecksumTask` 以 64 KiB 分块流式读取并在每个分块之间让出执行权，复用
RequestGroup 的 pause/remove/halt 生命周期取消和完整性 worker；内存下载仍使用
原有内存校验路径，不改变数据所有权。生产 core 路径已不再直接调用
`checksum::checksum::verify_file`。
...
该 checkpoint 关闭磁盘 whole-file checksum 绕过统一可取消 dispatcher 的生产路径
差距，但不等同于完整跨协议真实服务 E2E 或第三方客户端互操作验收；内存路径、
Metalink piece hash、BitTorrent piece 生命周期和最终 workspace 验收仍需继续验证。
当前阶段保持 `phase-2-core-domain` (`in_progress`)，整体迁移保持 `PARTIAL`。

## 2026-08-18 SFTP max-tries 总尝试次数检查点

新增 Rust-owned SFTP E2E：真实 SSH/SFTP fixture 对缺失远端文件持续返回
`SSH_FX_NO_SUCH_FILE`，配置 `max-tries=2`、`max-file-not-found=3` 和零等待，
生产命令最终返回 `ResourceNotFound`，且 fixture 恰好收到两次 `STAT`。这补齐了
SFTP not-found retry 遵守公开 `max-tries` 总尝试次数语义的直接证据，并与已有
`max-file-not-found` 阈值测试区分开。
...
该 checkpoint 只关闭 SFTP not-found retry 的总尝试次数证据；跨协议错误组合、
第三方/原版客户端互操作、bindings、实测性能和最终 workspace 验收仍未完成。

## 2026-08-18 FTP max-tries 总尝试次数检查点

新增 Rust-owned FTP E2E：真实本地 TCP fixture 接受并关闭每次 control connection，
配置 `max-tries=2` 和零等待后，生产命令在持续临时连接失败下恰好发起两次
control connection，并返回失败。这补齐了 FTP transient connection retry 遵守公开
`max-tries` 总尝试次数语义的直接证据。
...
该 checkpoint 只关闭 FTP transient connection retry 的总尝试次数证据；跨协议
错误组合、第三方/原版客户端互操作、bindings、实测性能和最终 workspace 验收仍未完成。

## 2026-08-18 真实 App 重启执行 paused Metalink graph 检查点

补充 Rust-owned 进程级回归：先将依赖 gated 的 Metalink metadata/payload graph
置为 paused 并保存 session，再创建全新的 `App` 与 `DownloadEngine`。恢复后确认
metadata 和 payload 都保持 paused，通过持久化 metadata GID 同时 unpause 两组，
随后实际重新获取一次 torrent metadata，完成 metadata 与 zero-length payload，
并验证最终输出文件已创建。
...
该 checkpoint 关闭 paused Metalink graph 在 fresh App 重启后的真实执行切片；
更广泛跨协议错误组合、第三方/原版客户端互操作、bindings、实测性能和最终
workspace 验收仍未完成。当前阶段保持 `phase-2-core-domain` (`in_progress`)，
整体迁移保持 `PARTIAL`。

## 2026-08-18 active-transfer force-remove 跨协议检查点

补充真实 Rust-owned active-transfer 回归：HTTP、FTP、SFTP 慢速下载在已有部分
进度和控制文件后收到 `ForceRemoveDownload`，三种生产命令都及时结束，最终
RequestGroup 状态为 `Removed`，且部分输出和 `.aria2` checkpoint 均保留。该路径
覆盖了 C API/RPC `force=true` 所使用的强制移除语义，与 `ForceHaltAll` 的可恢复
`InProgress` 语义保持区分。
...
该 checkpoint 关闭 HTTP/FTP/SFTP active-transfer force-remove 持久化切片；更广泛
的跨协议错误组合、第三方/原版客户端互操作、bindings、实测性能和最终 workspace
验收仍未完成。当前阶段保持 `phase-2-core-domain` (`in_progress`)，整体迁移保持
`PARTIAL`。

## 2026-08-18 FTP/SFTP retry-wait remove 跨协议检查点

补充真实 Rust-owned 跨协议回归：FTP 控制连接被本地 fixture 接受后立即关闭，
SFTP 首次 `STAT` 返回 `SSH_FX_NO_SUCH_FILE`，两者均进入五秒 retry-wait；随后
RequestGroup 收到 remove，生产命令都能及时返回 `Download cancelled by user`，且
不会发起第二次 FTP control connect 或 SFTP `STAT`。这验证了 remove 与 pause 共用的
事件驱动 retry waiter 以及协议错误映射。
...
该 checkpoint 关闭 FTP/SFTP retry-wait remove 切片；活动传输期间更广泛的
remove/halt 组合、第三方/原版客户端互操作、bindings、实测性能和最终 workspace
验收仍未完成。当前阶段保持 `phase-2-core-domain` (`in_progress`)，整体迁移保持
`PARTIAL`。

## 2026-08-18 真实 App 重启执行 nonzero Metalink graph 检查点

新增 Rust-owned 进程级回归：预写两片 nonzero payload、匹配 Rust `A2CF`
checkpoint（torrent marker、info-hash、piece length、完整 bitfield），保存标准
Metalink graph session，创建全新的 `App` 和 `DownloadEngine`，恢复 graph 后通过本地
HTTP fixture 重新获取 torrent metadata 并执行完成。测试确认 metadata 只请求一次，
metadata/payload 都进入 stopped `Complete`，且已有 payload 字节保持不变。

该场景证明恢复后的 session bitfield 与输出文件、匹配 sidecar 能共同驱动 nonzero
BitTorrent payload 的完成路径；本次没有生产代码修复。Rust-owned 验证：
...
该 checkpoint 关闭标准 Metalink graph 在真实 App 重启后的 nonzero payload 执行切片；
paused graph 的真实执行、更广泛跨协议错误组合、第三方/原版客户端互操作、bindings、
实测性能和最终 workspace 验收仍未完成。当前阶段保持 `phase-2-core-domain`
(`in_progress`)，整体迁移保持 `PARTIAL`。

## 2026-08-18 真实 App 重启执行标准 Metalink graph 检查点

新增 Rust-owned 进程级回归：先保存一个标准 Metalink torrent graph 的 session，
再创建全新的 `App`、初始化 `DownloadEngine`、恢复 metadata/payload 两个 group，
通过本地 HTTP fixture 下载合法的零长度 torrent metadata，并运行恢复后的 graph
直到 metadata 与 payload 都进入 stopped `Complete` 结果。metadata 只请求一次，
零长度 BitTorrent payload 也会创建最终空输出文件，不进入 tracker/peer discovery。

该场景同时修复了 zero-length BT 命令在完成前直接返回、导致单文件或多文件零长度
条目不落盘的问题；输出目录和文件创建仍由 core 的 Rust-native 文件生命周期负责。
...
该 checkpoint 只关闭标准 Metalink graph 在真实 App 重启后的 active/waiting 执行
和 zero-length 输出切片；paused graph、非零 payload 的完整重启恢复、更广泛跨协议
错误组合、第三方/原版客户端互操作、bindings、实测性能和最终 workspace 验收仍未
完成。当前阶段保持 `phase-2-core-domain` (`in_progress`)，整体迁移保持 `PARTIAL`。

## 2026-08-18 标准 Metalink graph session 重启检查点

标准 Metalink torrent graph 的 session 恢复现在会把已序列化的
BitTorrent piece bitfield、piece 数量、piece 长度和 info-hash 恢复到等待
metadata dependency 的 payload group。metadata 在重启后重新下载并解析时仍可
覆盖这些字段为经过校验的实际值；session wire 字段和 graph GID/metadata identity
保持不变。
...
该 checkpoint 只关闭标准 Metalink graph 重启时持久化 BT piece 状态的切片；真实
进程重启后的 paused/active graph 执行、更广泛跨协议错误组合、第三方/原版客户端
互操作、bindings、实测性能和最终 workspace 验收仍未完成。当前阶段保持
`phase-2-core-domain` (`in_progress`)，整体迁移保持 `PARTIAL`。

## 2026-08-18 paused Metalink graph 生命周期检查点

`RequestGroupMan` 现在只在确认标准 Metalink graph 关系
(`metadata.belongs_to(payload)` 且 `payload.metadata_info.gid()` 指向 metadata)
后，才把单个持久化 metadata GID 的 pause、force-pause 和 unpause 同步到
metadata/payload 两个 Rust group。session 恢复后不会再出现 metadata 已恢复而
payload 仍 paused、导致 dependency graph 无法继续的半暂停状态；普通下载和无
metadata provenance 的 follow child 保持独立生命周期。
...
该 checkpoint 只关闭恢复后标准 Metalink graph 的 pause/force-pause/unpause 协调
切片；真实进程重启执行、更广泛跨协议错误组合、第三方/原版客户端互操作、
bindings、实测性能和最终 workspace 验收仍未完成。当前阶段保持
`phase-2-core-domain` (`in_progress`)，整体迁移保持 `PARTIAL`。

## 2026-08-18 HTTP、metadata 与并发 retry-wait 生命周期检查点

顺序 HTTP retry wait 现在会先注册所属 `RequestGroup` 的生命周期 waiter，
再检查 pause/remove 状态，避免状态检查和开始计时之间丢失唤醒。内存 metadata
源的 404 重试复用同一顺序。并发 HTTP range 下载的 429/503 自适应冷却也改为
事件驱动的生命周期等待，pause/remove 会及时结束冷却并收口命令。`retry-wait`、
重试次数、HTTP 状态码、配置项和 wire 契约保持不变。
...
该 checkpoint 只关闭 HTTP/metadata retry-wait 和并发自适应冷却的
pause/remove 切片；FTP/SFTP remove/halt 组合、更广泛跨协议生命周期、第三方/原版
客户端互操作、bindings、实测性能和最终 workspace 验收仍未完成。当前阶段保持
`phase-2-core-domain` (`in_progress`)，整体迁移保持 `PARTIAL`。

## 2026-08-18 HTTP、FTP、SFTP shutdown-halt E2E 检查点

生产 `DownloadEngine` 现在有真实协议数据任务的 graceful
`HaltAll(ShutdownSignal)` 回归：HTTP、FTP、SFTP 命令都会及时退出，保留部分
payload 和 control file，保持 `ShutdownSignal` 而不是错误地变成 user removal，
并生成可供 session 恢复的 `InProgress` stopped snapshot。该检查点只覆盖 graceful
shutdown-halt；force-halt 和更广泛跨协议错误组合仍未关闭。
...
该 checkpoint 只关闭 graceful shutdown-halt 持久化切片；force-halt、session graph
重启变体、跨协议错误/retry 组合、第三方/原版客户端互操作、bindings、实测性能和
最终 workspace 验收仍未完成。当前阶段保持 `phase-2-core-domain` (`in_progress`)，
整体迁移保持 `PARTIAL`。

## 2026-08-18 HTTP、FTP、SFTP force-halt E2E 检查点

生产 `DownloadEngine` 现在有真实 HTTP、FTP、SFTP 活动传输期间的
`ForceHaltAll(ShutdownSignal)` 回归。forced halt 会终止协议任务，保留
control-file 的恢复边界，保持 `ShutdownSignal` 而不是转成 user removal，并生成
`InProgress` 结果供恢复。该路径使用 engine 的 cancellation 和 completion accounting，
与 graceful halt 检查点分开验证。
...
该 checkpoint 只关闭生产 forced shutdown-halt 持久化切片；session graph 重启变体、
跨协议错误/retry 组合、第三方/原版客户端互操作、bindings、实测性能和最终
workspace 验收仍未完成。当前阶段保持 `phase-2-core-domain` (`in_progress`)，
整体迁移保持 `PARTIAL`。

## 2026-08-18 FTP retry-wait pause E2E 检查点

新增 Rust-owned FTP 真实生命周期回归：临时 TCP listener 接受 FTP 控制连接后
立即断开，使生产 `FtpDownloadCommand` 进入临时网络错误和 5 秒 `retry-wait`；
随后通过同一个 `RequestGroup` 发出 pause，命令在等待期间被 lifecycle notification
唤醒并返回 `Download paused`，而不是睡完整个重试间隔。测试不读取或运行
`aria2_original`，也不改变 FTP 配置、默认值或协议 wire。
...
该 checkpoint 只关闭真实 FTP 控制连接临时失败后的 retry-wait pause 切片；
FTP remove/halt 组合、SFTP remove/halt 组合、第三方/原版客户端互操作、
bindings、实测性能和最终 workspace 验收仍未完成。当前阶段保持
`phase-2-core-domain` (`in_progress`)，整体迁移保持 `PARTIAL`。

## 2026-08-18 Integrity dispatcher pre-cancel 生命周期检查点

`enqueue_with_outcome_for_group` 现在先把校验 entry 放入
`CheckIntegrityMan`，再观察所属 `RequestGroup` 的 pause/remove/halt 状态。
此前如果 group 在该 future 首次轮询前已经 pause，取消逻辑会先调用
`cancel_gid`，随后才把 entry 入队，等待方无法再收到取消完成通知。现在 queued
和 active entry 都由同一个 manager 负责收口；初始已暂停任务也会立即返回
`Download paused`，不会遗留校验任务或 manager 计数。
...
该 checkpoint 只关闭校验 dispatcher 的初始 group 生命周期竞态；生产 HTTP 和
BitTorrent owner 的更广泛跨协议 pause/remove/retry 组合、外部互操作、bindings、
实测性能和最终 workspace 验收仍未完成。当前阶段保持
`phase-2-core-domain` (`in_progress`)，整体迁移保持 `PARTIAL`。

## 2026-08-18 Terminal DownloadResult/force-save session 检查点

`RequestGroup::create_download_result` 现在把当前 `DownloadContext` 的文件路径、
长度、完成长度、selected 状态以及每个 URI 的 waiting/used/spent 状态写入 stopped
snapshot，同时保留任务结束时的 effective option snapshot 和 BitTorrent 字段。
生产 `SaveSessionCommand` 会把 stopped results 交给 session serializer；finished/
removed 结果遵循 `force-save`，not-found 结果遵循 `save-not-found`，shutdown
in-progress 结果可恢复。序列化时 waiting URI 先写、已使用 URI 后写并去重，terminal
结果被写成 waiting session entry，以便恢复路径重新调度而不会被 complete/removed
状态过滤掉。应用 shutdown 的 `ActiveSessionManager` 现在也传递 stopped results，
因此实际 App 保存路径与引擎 `SaveSessionCommand` 保持同一策略。
...
cargo test -p aria2-core --all-features --lib session::active_session -- --test-threads=1
  6 passed, 0 failed
cargo test -p aria2 --all-features --lib app::tests::test_force_saved_stopped_result_survives_app_session_restart -- --exact --test-threads=1
  1 passed, 0 failed
...
cargo test -p aria2-core --all-features --lib --tests -j 1 --quiet -- --test-threads=1
  aria2-core library: 3489 passed, 0 failed, 1 ignored
cargo test -p aria2-rpc --all-features --lib -- --test-threads=1
  233 passed, 0 failed, 0 ignored
cargo test -p aria2-core --all-features --test test_e2e_bittorrent_download -- --test-threads=1
  36 passed, 0 failed, 2 ignored
cargo test -p aria2-core --all-features --test test_e2e_ftp_proxy -- --test-threads=1
...
首次全量 integration target 顺序执行在 Windows 上遇到 `os error 32`，FTP 测试
二进制未启动；随后单独重跑通过。该 checkpoint 关闭标准 App shutdown -> restart
的 stopped result session 序列化（包括 `.gz` session）、`force-save`/`save-not-found`
策略切片，但更广泛
session restart 变体、跨协议生命周期、原版/第三方互操作、bindings、实测性能和
最终 workspace 验收仍未完成。
当前阶段保持 `phase-2-core-domain` (`in_progress`)，整体迁移保持 `PARTIAL`。

## 2026-08-18 `.gz` Session 文件检查点

Rust session serializer 现在按 session 文件的精确小写 `.gz` 后缀透明选择
gzip 编解码。`load_from_file` 可读取 aria2_original `GZipFile` 生成的 gzip
内容；`save_to_file` 和 `save_to_file_with_entries` 会先完成压缩，再将结果写入
现有的 `.sess.tmp` 临时文件并原子改名。纯文本 session 路径保持原有格式和写入
流程，损坏或非 gzip 内容不会被静默当作空 session 接受。
...
cargo clippy -p aria2-core --all-targets --all-features -- -D warnings
  PASS
rustfmt --edition 2024 --check aria2-core/src/session/session_serializer/file_io.rs aria2-core/src/session/session_serializer/tests.rs
  PASS
git diff --check
  PASS
~~~

该 checkpoint 只关闭透明 `.gz` session 文件读写和损坏输入报告切片；stopped
result 的 `force-save`/`save-not-found` 策略及标准 App shutdown→restart 覆盖已在
前一检查点记录。更广泛的 session restart 变体、完整重启互操作、跨协议生命周期、
bindings、实测性能和最终 workspace 验收仍未完成。当前阶段保持
`phase-2-core-domain` (`in_progress`)，整体迁移保持 `PARTIAL`。

## 2026-08-18 uTP readiness 与动态限速事件驱动检查点

uTP 的异步接收路径现在为同步协议状态保留惰性 Tokio UDP readiness handle。
`UtpPeerConnection` 先在 socket 状态机中尝试读取，确认没有可用 payload 后再等待
真实的 UDP readable 事件；不再用固定 sleep 反复调用 `recv`。uTP BitTorrent
消息使用持久 `BytesMut` 累积半帧，分片到达后继续等待并返回完整长度前缀 frame，
不会把半帧误报成空读或丢弃到达数据。同步 uTP API 和现有连接状态机保持不变。

`TokenBucket::acquire` 现在同时等待 token refill deadline 和动态配置通知。
`set_rate`、`set_unlimited` 会唤醒阻塞 acquire，因此 `changeOption` 的限速变化不必
等完旧的固定 sleep；`ThrottledWriter` 仍按 chunk 交错限速和磁盘写入，避免大型
transport chunk 把所有写入放在一次长等待之后。
...
这只关闭 uTP 异步接收和动态限速唤醒的空转等待切片；DHT/tracker announce、retry
backoff、keepalive、Happy Eyeballs、UI throttle，以及没有平台事件 API 的
process-wait fallback 仍保留其协议或平台语义定时器。完整 workspace 验收、跨平台
网络实测、原版/第三方互操作和端到端吞吐基准仍未完成，整体迁移保持 `PARTIAL`。

## 2026-08-18 Bitfield 字节级扫描检查点

`PieceStatMan` 的 add/subtract 现在按 bitfield 的置位 bit 更新计数，
`update_piece_stats` 只遍历 new/old 的差异 bit；`BitfieldMan` 的 missing 查询和
stream selection 的 combined unavailable 构造改为按 byte 直接组合，避免每个 piece
重复做除法、边界检查和临时 `set_bit`。最后一个 byte 的无效 trailing bits 仍按逻辑
piece 数屏蔽，短 peer/ignore bitfield 仍按缺失字节处理为 0。

Rust-owned benchmark（Windows release，Criterion `sample-size=10`、每项
`measurement-time=1s`，同一工作树前后对照）：

~~~text
PieceStatMan add sparse, 50,000: 约 60.1 us -> 19.8 us
PieceStatMan update changed bits, 50,000: 约 58.5 us -> 39.3 us
BitfieldMan all missing, 50,000: 约 63.4 us -> 4.3 us
BitfieldMan all missing unused, 50,000: 约 76.9 us -> 4.8 us
BitfieldMan sparse selection, 50,000: 约 194.9 us -> 75.0 us
~~~

对应 segment scan regression tests、BitTorrent E2E 和 all-target Clippy 均通过。
这些是 Rust 算法级结果，不是完整下载吞吐提升，也未与 `aria2_original` 做同机
对照；跨平台数据分布和最终 workspace 验收仍保持 `PARTIAL`。

## 2026-08-18 Session spent URI 持久化检查点

`SessionSerializer::group_to_entry` 不再读取 `RequestGroup` 创建时的静态 URI
列表，而是从当前 `DownloadContext` 合并 remaining URI 与 spent URI。输出保持
aria2_original 的 remaining-first 顺序，并对两个集合去重；恢复时这些 URI 仍作为
普通 session URI 重新进入任务，因此已尝试过的镜像不会在重启后丢失。
...
该 checkpoint 只关闭当前 remaining/spent URI 的 session 序列化切片；stopped result
的 `force-save`/`save-not-found` 策略和 `.gz` session 文件读写已在相邻检查点记录。
更广泛的 session restart 变体、完整重启互操作、跨协议生命周期、bindings、实测性能
和最终 workspace 验收仍未完成。当前阶段保持 `phase-2-core-domain` (`in_progress`)，
整体迁移保持 `PARTIAL`。

## 2026-08-18 BitTorrent progress manager bitfield resume checkpoint

`BtDownloadCommand::set_progress_manager` 现在真正持久化 piece picker 的
completed bitfield，而不是写入空 bitfield。恢复时，C++ 兼容的二进制或历史文本
progress 文件只作为 Rust-owned A2CF checkpoint 之后的 fallback；只有 payload
存在、没有更新的 A2CF 进度、未执行 integrity check、未启用
`bt-seed-unverified`，且 info hash、torrent 标记、piece length、total size、piece
count、bitfield 长度和 trailing bits 都匹配时，已完成 pieces 才会注入 picker。
保存失败不会错误地推进下一次保存时间；保存的 bitfield 由 picker 在 piece 完成
后导出，并在 payload 写回后持久化。
...
该 checkpoint 关闭的是 progress manager 的 bitfield 保存、严格布局校验和实际
legacy resume picker 接入；Rust A2CF 优先级、integrity 语义、跨协议生命周期、
原版/第三方互操作、bindings、实测性能和最终 workspace 验收仍未完成。当前阶段
保持 `phase-2-core-domain` (`in_progress`)，整体迁移保持 `PARTIAL`。

## 2026-08-18 BitTorrent 事件驱动 pipeline 与查询快路径检查点

普通 BitTorrent piece 下载现在使用每个 peer 有界的 block pipeline：单 peer
最多保留 6 个 outstanding request，通过 `FuturesUnordered` 按到达事件处理
乱序 `PIECE`、`Reject`、超时和断连，并把失败请求重新排队。endgame 的重复
请求也同时等待多个 peer，不再以 round-robin 顺序等待慢连接。TCP 和 MSE
帧读取使用连接内持久缓冲，reader future 被取消后，已收到的半帧会在下一次
读取中继续消费，不会破坏后续消息边界。

BT 调度器不再为每次普通选片分配全 1 bitfield。`PiecePicker` 维护按
`(availability, index)` 排序的 rarest 顺序和游标；可无限制选片时，完成或
不可用前缀只被跳过一次，peer bitfield 受限的选择仍保留线性扫描语义。RPC
的 `tellActive`/`tellWaiting` 直接读取对应调度索引，稳定态查询跳过 canonical
索引的重复扫描和去重集合分配。
...
cargo test -p aria2-protocol --features bittorrent --lib bittorrent::peer::connection::tests::test_read_message_preserves_partial_frame_after_cancellation -- --exact --test-threads=1
  1 passed, 0 failed
cargo test -p aria2-protocol --features bittorrent --lib bittorrent::peer::encrypted_connection::tests::test_read_message_preserves_partial_frame_after_cancellation -- --exact --test-threads=1
  1 passed, 0 failed
cargo bench -p aria2-protocol --features bittorrent --bench sequential_picker_bench -- rarest_selection --noplot
  rarest_selection/legacy_linear_scan: 1000=1.14 us, 10000=11.35 us, 50000=54.96 us (median)
  rarest_selection/sorted_cursor:      1000=0.231 us, 10000=1.17 us, 50000=2.13 us (median)
~~~

这些数值是当前 Rust 工作树的算法级对照，不是完整 BT 吞吐提升百分比；基准
包含 picker 构造和状态设置的可比场景，未与 `aria2_original` 做同机实测，不能
据此宣称整体性能优于原版。并发 reader 的安全性只覆盖 TCP/MSE 帧边界，uTP
仍属于独立接收缓冲路径。整体 BitTorrent、外部客户端互操作、bindings、跨平台
性能和最终 workspace 验收仍保持 `PARTIAL`。

## 2026-08-18 BitTorrent 控制文件 piece length 检查点

原版 `DefaultBtProgressInfoFile` 会在控制文件中保存并校验
`pieceLength`。Rust-owned A2CF 现在为 BitTorrent checkpoint 增加可选的
piece-length 字段：新保存的 checkpoint 会写入当前 torrent piece length，恢复时
若字段存在且不匹配则丢弃 stale sidecar，而不是把旧 bitfield 套到新的布局上。

没有该字段的历史 A2CF 仍可恢复，但只有在 total length、payload、bitfield、
trailing bits 和 info hash 都匹配时才接受，并会在下一次持久化时补齐字段。这个
兼容决策利用了合法 torrent 的 info hash 覆盖整个 info dictionary（其中包含
piece length），不复制原版内部控制文件布局，也不声称支持跨 piece layout 的
bitfield 转换；当前 Rust checkpoint 只保存已验证的完整 pieces。

Rust-owned 验证：

~~~text
cargo test -p aria2-core --all-features --lib filesystem::control_file::tests -- --test-threads=1
  14 passed, 0 failed
cargo test -p aria2-core --all-features --lib engine::bt_checkpoint::tests -- --test-threads=1
  6 passed, 0 failed
cargo test -p aria2-core --all-features --lib --tests -j 1 --quiet -- --test-threads=1
  aria2-core library: 3477 passed, 0 failed, 1 ignored
  all aria2-core integration test targets passed
cargo clippy -p aria2-core --all-targets --all-features -- -D warnings
  PASS
cargo fmt --all -- --check
...
这个检查点只关闭 Rust-owned BitTorrent checkpoint 的 piece-length 持久化与 stale
sidecar 校验切片；原版控制文件格式互操作、跨 piece layout 转换、跨协议生命周期、
bindings、实测性能和最终 workspace 验收仍未完成。当前阶段保持
`phase-2-core-domain` (`in_progress`)，整体迁移保持 `PARTIAL`。

## 2026-08-18 多文件完整性校验截断条目检查点

`MultiFileChunkValidator::read_piece` 现在按原版 piece 校验语义处理已有物理文件的
EOF：声明长度大于实际长度时保留已读字节，让当前 piece 的摘要自然不匹配，随后
继续完成校验并进入正常的 failed-piece 重下载路径；打开、定位和实际读取系统错误
仍然作为 I/O 错误返回。此前的 `read_exact` 会把同一场景直接报告为 `early eof`，
导致任务无法走完整性失败恢复路径。

这个语义与 `aria2_original/src/IteratableChunkChecksumValidator.cc` 的
`validateChunk`/`digest` 相符：原版捕获可恢复的读取异常、清除当前 piece 并推进
到下一个 piece。新增 Rust-owned 回归只使用临时多文件 fixture，确认完整 piece
仍然验证通过，截断 piece 被记录为失败，不依赖原版源码运行测试。

Rust-owned 验证：

~~~text
cargo test -p aria2-core --all-features --lib checksum::check_integrity::man::tests::test_multi_file_task_truncated_existing_file_is_mismatch -- --exact --nocapture
  1 passed, 0 failed
cargo test -p aria2-core --all-features --lib checksum::check_integrity::man::tests -- --test-threads=1
  15 passed, 0 failed
cargo test -p aria2-core --all-features --test test_e2e_bittorrent_download -- --test-threads=1
  35 passed, 0 failed, 2 ignored
cargo test -p aria2-core --all-features --lib --tests -j 1 --quiet -- --test-threads=1
  aria2-core library: 3475 passed, 0 failed, 1 ignored
  all aria2-core integration test targets passed
cargo clippy -p aria2-core --all-targets --all-features -- -D warnings
  PASS
cargo fmt --all -- --check
...
本检查点只关闭已有多文件 payload 的物理截断校验恢复切片；更广泛的跨协议
生命周期组合、存储/session 边界、第三方/原版客户端互操作、bindings、实测性能
和最终 workspace 验收仍未完成。当前阶段保持 `phase-2-core-domain`
(`in_progress`)，整体迁移保持 `PARTIAL`。

## 2026-08-18 Metalink 依赖阻塞删除检查点

`RequestGroupMan::remove_reserved_group` 现在与 `aria2_original` 的
`removeDownload` 语义一致：仍处于未解析依赖状态的 reserved group 不能被普通
`remove` 或 `force-remove` 独立删除。Metalink metadata/payload graph 必须先让
metadata prerequisite 进入终止状态，由依赖解析统一决定 payload 是注入 torrent
context、切换到 direct mirror fallback，还是记录终止错误；这样不会删除 payload
后留下孤立的 metadata prerequisite。

Rust-owned 回归使用内存构造的 Metalink torrent graph，确认两种删除请求都返回
`cannot be removed now`，metadata 和 payload 仍在 manager 中且没有提前写入 stopped
result。这个行为与 `aria2_original/src/RpcMethodImpl.cc` 和
`aria2_original/src/aria2api.cc` 的 reserved 依赖检查相符；测试运行时只使用 Rust
自己的 graph fixture，不依赖原版源码。
...
cargo test -p aria2-core --all-features --test test_e2e_metalink_lifecycle -- --test-threads=1
  14 passed, 0 failed, 2 ignored
cargo test -p aria2-core --all-features --lib --tests -j 1 --quiet -- --test-threads=1
  aria2-core library: 3473 passed, 0 failed, 1 ignored
  all aria2-core integration test targets passed
cargo clippy -p aria2-core --all-targets --all-features -- -D warnings
  PASS
cargo fmt --all -- --check
...
本检查点只关闭 Metalink 依赖未解析时的 reserved remove/force-remove 生命周期
切片；更广泛的 Metalink follow graph、跨协议生命周期、第三方/原版客户端互操作、
bindings、实测性能和最终 workspace 验收仍未完成。当前阶段保持
`phase-2-core-domain` (`in_progress`)，整体迁移保持 `PARTIAL`。
...
The Rust-owned SFTP fixture now counts remote `STAT` requests, allowing a
production command test to observe the first `SSH_FX_NO_SUCH_FILE` response
before changing lifecycle state. With `max-file-not-found=2` and a configured
five-second `retry-wait`, pausing the owning `RequestGroup` wakes the SFTP
retry wait promptly and prevents a second `STAT` attempt. The retry future
registers its `Notify` waiter before checking state, closing the check-to-listen
notification race without changing public option names, defaults, session
format, RPC wire values, protocol wire values, or product identity.
...
`EngineCommand::ForceHaltAll` now removes groups that have not yet been
promoted from the request manager's reserved queue. The manager marks each
group as `Removed`, unregisters it from the canonical group index, and records
the stopped result before the engine termination check. Active groups remain on
the existing force-halt path so their command handles can be cancelled and
drained normally.
