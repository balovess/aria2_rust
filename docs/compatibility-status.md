...
| Latest focused unknown-length storage verification | `UnknownLengthPieceStorage::complete_piece` now rejects stale completions when no piece is checked out and rejects nonzero piece indexes, preventing a late post-cancellation notification from marking the download complete. The Rust-owned `segment::unknown_length_piece_storage` module reports `17 passed, 0 failed` using the sole default `target`; broader cross-protocol lifecycle and restart coverage remains open. |
