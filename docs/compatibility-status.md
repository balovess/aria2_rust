...
Binary package version: 0.3.4
Library package versions: aria2-core 0.3.4, aria2-protocol 0.3.2, aria2-rpc 0.3.2
Public product identity: aria2-rust 0.3.4
...
| Latest focused BitTorrent lifecycle verification | Engine `ForceHaltAll(ShutdownSignal)` now preserves the active BT command's writer flush and A2CF checkpoint by allowing the constructed command to observe the `RequestGroup` lifecycle. The new engine E2E and full BT target report `1` and `37 passed, 0 failed, 2 ignored`; HTTP/FTP/SFTP lifecycle targets report `43/0/2`, `43/0/2`, and `30/0/2`; the core all-features library aggregate reports `3596 passed, 0 failed, 1 ignored`; core all-targets/all-features Clippy, formatting, and `git diff --check` pass. |
| Latest focused file-allocation cancellation verification | `FileAllocationMan::remove_from_queue` now cancels removed entries and notifies their `enqueue_*` waiters with the shared cancellation error instead of silently dropping the sender. Active `Trunc`, `Falloc`, and `Mmap` allocation paths now check cancellation before and after their one-shot operation. The Rust-owned `filesystem::file_allocation_man` module reports `18 passed, 0 failed` using the sole default `target`; broader force-halt allocation and cross-protocol lifecycle coverage remains open. |
| Latest focused force-halt allocation ordering verification | `EngineCommand::ForceHaltAll` now cancels file allocations for running GIDs before waiting for protocol tasks, then repeats cancellation after task teardown to cover late enqueue races. A waiter blocked in the production allocation entrypoint now wakes before the one-second force-shutdown timeout; the Rust-owned `engine::engine_loop::tests::force_halt_wakes_file_allocation_waiter_before_protocol_timeout` regression and the full engine-loop module report `1` and `24` passed, with zero failures. The production multi-file BitTorrent `prealloc` force-halt E2E reports `1 passed, 0 failed`; broader cross-protocol force-halt coverage remains open. |
| Latest focused unknown-length storage verification | `Piece` now carries a clone-preserved internal checkout identity that is excluded from equality and persistence; `UnknownLengthPieceStorage::complete_piece` rejects no-active-piece, nonzero-index, and older-checkout completions, preventing a late post-cancellation notification from completing a newer checkout. `SegmentMan` tracking now retains that identity, requires the matching `(CUID, piece index, checkout)` for concrete completion/cancellation, rejects untracked completions before touching storage, preserves in-flight tracking, and suppresses `have` advertisement when storage rejects a completion. The Rust-owned `segment::unknown_length_piece_storage` and `segment::segment_man` modules report `18` and `36` passed, respectively, with zero failures using the sole default `target`; broader cross-protocol lifecycle and restart coverage remains open. |
| Latest focused Metalink batch lifecycle verification | `RequestGroupMan::pause_all`, `force_pause_all`, and `unpause_all` now apply lifecycle transitions to both metadata and payload groups in a standard Metalink request graph, with GID-based de-duplication across the canonical group index. Rust-owned `request_group_man::tests::batch_pause_operations_cover_both_metalink_graph_groups` reports `1 passed, 0 failed`; core all-features library Clippy and formatting pass using the sole default `target`. Broader cross-protocol lifecycle and restart coverage remains open. |
| Latest focused completion-dependency failure verification | Generic `CompletionDependency` consumers now receive a terminal error when their prerequisite reaches `Error` or `Removed`, instead of remaining indefinitely in the reserved queue. The dependent is removed from the canonical index and recorded in stopped results with `UnknownError`; the existing successful-completion and BitTorrent fallback paths remain unchanged. Rust-owned `request_group_man` reports `43 passed, 0 failed`; core all-features Clippy and formatting pass using the sole default `target`. Broader dependency-graph and cross-protocol lifecycle coverage remains open. |
| Latest verification | 2026-08-28 current-host refresh: `cargo test -j 1 -p aria2-core --all-features --lib http::client_identity -- --test-threads=1` reports `22 passed, 0 failed`, including Rust-owned AES-128-GCM/AES-256-GCM empty-password PFX identity and unknown-additional-bag coverage; current Cargo manifest, lockfile, and metadata report `aria2-core` `0.3.4`; historical focused results remain retained below. |
| Latest current-host crate test refresh | On the current worktree, `cargo test -j 1 --workspace --all-features --lib --tests --no-fail-fast -- --test-threads=1` reports `aria2-core` `3603 passed, 0 failed, 1 ignored`; the CLI, protocol, RPC, library, integration, lifecycle, checksum, storage, DHT, BitTorrent, HTTP, FTP, SFTP, Metalink, stress, and performance targets all pass. Criterion benchmark binaries are intentionally excluded from this test-only invocation because forwarded test-thread arguments are not accepted by Criterion. The command uses only the sole default `target`. |
| Latest workspace library/integration refresh | Current-host `cargo test -j 1 --workspace --all-features --lib --tests --no-fail-fast -- --test-threads=1` completed successfully using the sole `target`; all executed CLI, core, protocol, RPC, lifecycle, checksum, storage, DHT, BitTorrent, HTTP, FTP, SFTP, Metalink, stress, and performance targets passed. Criterion benchmark binaries were intentionally excluded from this test-only invocation; ignored tests remain reported by their owning targets. |
| Latest bindings verification | Current-host `py -3.14 -m pytest -q -rA` reports `137 passed in 38.36s` after installing the declared Python dev extras; `npm test` reports `123 passed` and `npm run typecheck` passes in `bindings/nodejs`. This closes the current local Python/Node test and typecheck evidence only; platform-specific runs, package publication metadata, and complete external-client interoperability remain open. |
| Latest performance verification | Current-host fixed `test_e2e_throughput_regression` reports `562.5 MiB/s` for 16 segments and 10 MiB. Lifecycle-matched Criterion `positioned_write_bench` reports `1.7509 GiB/s` for `PositionedDiskWriter_concurrent_4x1MB` versus `2.2850 GiB/s` for `OldMutexDirectDiskAdaptor_4x1MB`; this is a Rust-internal Windows regression signal, not evidence of superiority. Profiling/optimization and an aria2_original baseline remain open. |
| Latest focused protocol rerun | FTP control-connection and SFTP not-found `max-tries=2` total-attempt E2E tests passed individually; active-transfer force-remove tests passed individually; full targets then passed with HTTP `43/0/2`, FTP `43/0/2`, and SFTP `30/0/2`; core library Clippy passes, while the current workspace format caveat is recorded in the current-host refresh row above |
| Latest focused DHT boundary verification | Inbound DHT queries carrying the local node ID are now dropped before method dispatch, matching the original receiver's local-node guard: no response, no good-node mark, and no routing-table reinsertion. BitTorrent completion now sends the actual TCP peer-listener port in DHT `announce_peer` and skips the announcement when no listener port exists. Rust-owned handler tests report `7 passed, 0 failed`; DHT announce-port regression reports `3 passed, 0 failed`; `dht_integration_tests` report `30 passed, 0 failed, 4 ignored`; protocol all-targets/all-features Clippy with `-D warnings` passes. Public DHT interoperability and original-client evidence remain open. |
| Latest focused cross-protocol verification | Current-host Rust-owned `cargo test -j 1 -p aria2-core --all-features --test deep_e2e_cross_protocol -- --test-threads=1` reports `18 passed, 0 failed, 2 ignored` for Metalink checksum/mirror behavior, FTP authentication, rate limiting, disk-write failure propagation, session restore, filename collision, and local DHT/LPD fixtures; no production behavior changed in this refresh; broader protocol error/retry combinations, live interoperability, measured performance, and final workspace acceptance remain open |
| Latest focused cross-protocol error mapping | Metalink direct-mirror and torrent-metaurl HTTP `401/407` responses now map to `HttpAuthFailed`, matching the sequential and segmented HTTP paths; the focused Metalink status/lifecycle module reports `9 passed, 0 failed`; core library Clippy and changed-file rustfmt pass. Broader cross-protocol retry combinations, live interoperability, measured performance, and final workspace acceptance remain open |
| Latest focused concurrent HTTP verification | The Rust-owned concurrent HTTP range target reports `14 passed, 0 failed`; it covers terminal 500/502 behavior, 503 retry after `retry-wait`, 504 retry, terminal 500 mirror failover, multi-mirror resume, pause/remove, and save-session checkpoint flushing. The core all-features library aggregate reports `3508 passed, 0 failed, 1 ignored`; core all-targets/all-features Clippy, full formatting, and `git diff --check` pass; the historical package version at that checkpoint was `0.3.2`, while current package metadata is `0.3.4`. |
| Latest focused retry/RPC verification | The corrected HTTP retry-policy targets report `test_error_network` `32 passed, 0 failed, 2 ignored` and `test_retry` `16 passed, 0 failed`; HTTP download E2E reports `43 passed, 0 failed, 2 ignored`; gap retry E2E reports `26 passed, 0 failed, 2 ignored`; RPC active/progress E2E reports `9 passed, 0 failed`. The tests now use the Rust-owned 503 retry fixture and the real reserved-to-active promotion path; HTTP 500 remains terminal. |
| Latest focused Metalink retry verification | Metalink direct mirror payloads and torrent metaurl metadata now honor total `max-tries` through the shared `RetryPolicy`; Rust-owned 504-then-success regressions make two requests and complete, while existing 404 threshold/zero-limit behavior remains green. The Metalink command slice reports `25 passed, 0 failed`; the Metalink lifecycle target reports `14 passed, 0 failed, 2 ignored`; core all-targets/all-features Clippy, formatting, and `git diff --check` pass. |
| Latest workspace aggregate caveat | The prior `cargo test --workspace --all-features --all-targets --no-fail-fast -- --test-threads=1` attempt reported 19 failed targets: 15 Criterion benchmark binaries rejected the test-only `--test-threads` argument, while four ordinary test targets exposed the stale 500 retry assertions or direct RPC scheduling-state setup. Those ordinary failures are corrected; `cargo test --workspace --all-features --lib --tests --no-fail-fast -- --test-threads=1` now passes. A benchmark-specific invocation remains open evidence. |
...
| Latest event-driven idle verification | The engine waits on commands, task completions, the earliest real maintenance deadline, or shutdown instead of scanning on a fixed idle tick. `ActivitySignal` combines `Notify` with a generation counter for RequestGroup lifecycle, progress, file-allocation, retry, and console observers; session/control-file persistence shares `AutoSaveCoordinator` deadlines; Console UI keeps only its 250 ms render throttle; native process waiting uses Windows events, Linux `pidfd`, and macOS/BSD `kqueue`, with a low-frequency fallback only on unsupported targets. Focused tests report engine loop `19 passed`, RequestGroup activity/manager `35 passed`, autosave coordinator `3 passed`, and Console UI `4 passed`, all with zero failures. Explicit SaveSession requests are retained, and BitTorrent checkpoint publication flushes the writer before persisting verified-piece state. |
| Latest focused Piece cache-ownership cleanup | Removed stale `WrDiskCache` TODOs from `aria2-core/src/segment/piece/piece_impl.rs`; `Piece` now documents that cache state belongs to the active writer. Core `segment::piece`-filtered library tests report `110 passed, 0 failed`; core library Clippy with `-D warnings` passes; `git diff --check` passes. Full workspace formatting remains blocked only by the existing `aria2-core/src/config/parser.rs:475` formatting diff. |
| Current verification caveat | The C API option-change test hang was caused by the test retaining an internal `RequestGroup` read lock across `aria2_rust_session_final`; the assertion is now scoped so finalize can acquire its write lock, and the focused C API and engine-loop suites pass. The current workspace all-features core library aggregate reports `3603 passed, 0 failed, 1 ignored`; the Goal remains `PARTIAL`. |
| Current status | `PARTIAL`; phase 1 is locked, but the final acceptance and stop conditions are not met |
| Latest focused TLS identity verification | The Rust-owned empty-password PKCS#12 adapter now falls back to PBES2 decoding for AES-128-CBC, AES-192-CBC, AES-128-GCM, and AES-256-GCM when `p12_q3` cannot decrypt the archive, walks encrypted safe contents through the same fallback, accepts plaintext `keyBag` PKCS#8 DER, accepts PBKDF2 HMAC SHA-224/SHA-384/SHA-512 PRFs through the existing `pkcs5` implementation plus the GCM path, and ignores unknown additional bags when a usable certificate and private key are present. The current-host `http::client_identity` module reports `22 passed, 0 failed` using the sole default `target`. The original contract requires a blank PKCS#12 import password; password-bearing archives are intentionally rejected rather than treated as a missing compatibility feature. Unsupported certificate representations and broader original-client/third-party HTTPS interoperability remain open. |
| Completed improvements | RequestGroup-scoped queued/active integrity cancellation for HTTP and BitTorrent pre-download checks; disk-based whole-file checksum validation for HTTP, Metalink, FTP, and SFTP now uses one bounded, lifecycle-aware integrity dispatcher while in-memory paths retain direct validation; existing multi-file integrity validation now treats a physically truncated entry as a failed piece and follows the normal re-download path; Rust-owned BitTorrent A2CF checkpoints now persist and validate piece length while preserving historical sidecars through info-hash identity; explicit pause/remove/halt regression coverage; real HTTP, FTP, and SFTP graceful and forced `HaltAll(ShutdownSignal)` tests now preserve `InProgress` resume state and control files; force-halt now cancels running GID-owned file allocations before waiting for protocol task teardown and repeats cancellation afterward for late enqueue races; BitTorrent normal pieces now use a bounded event-driven block pipeline and endgame peers are read concurrently; TCP/MSE frame reads preserve partial data across cancellation; unrestricted rarest-first selection uses a sorted cursor instead of repeated linear scans; active/waiting RPC status uses direct scheduling indexes and a stable-state fast path; BitTorrent block reads now interrupt promptly on a real halt and leave a durable checkpoint, while non-halt lifecycle notifications such as save-session retry the current piece instead of terminating the command; BitTorrent seeding now races the owning RequestGroup lifecycle, cancels and drains the seed manager on pause/remove/halt, preserves stopped announce cleanup, and returns a lifecycle error before finalization; FTP and SFTP in-flight reads now observe lifecycle notifications and use bounded connection-drop cleanup on halt; FTP and SFTP retry-wait remove regressions now prove lifecycle wake-up and no duplicate protocol attempt; reserved-queue promotion now skips blocked head groups so later runnable groups are not starved under a finite concurrency limit; sequential HTTP retry waits, concurrent 429/503 adaptive cooldowns, FTP/SFTP retry waits, in-memory metadata retries, and Metalink direct mirror and torrent metaurl retries now use event-driven RequestGroup lifecycle notifications and match the original 504/502/503 status policy; pre-constructed groups now preserve `follow-metalink=mem` and do not materialize the source document; unresolved Metalink metadata dependencies now reject reserved payload `remove` and `force-remove` until the graph can resolve or fail coherently; HTTP-date parsing and formatting now share one validated RFC 7231/RFC 6265-compatible implementation; the standalone `HttpResponseProcessor` now honors configured retry-wait and preserves retryable/fatal HTTP classifications; command failures now carry the last concrete connection context, allowing the engine to mark only an observed HTTP/FTP peer bad for network/timeout failures; session serialization of configured `certificate` and `private-key` paths; current session serialization now preserves remaining and spent URIs in original order with duplicate suppression; stopped `DownloadResult` snapshots now retain file/URI progress, effective task options, and `SaveSessionCommand` plus App shutdown serialization now apply `force-save`/`save-not-found` policy; standard App shutdown/restart restores a force-saved terminal result as a waiting task; standard Metalink graph session restart now restores persisted BT piece bitfield and metadata; restored standard Metalink graph lifecycle now coordinates metadata/payload pause, force-pause, and unpause by the persisted metadata GID; restored standard Metalink graphs now execute through a fresh App/engine after session restart for both zero-length and pre-existing nonzero payloads; omitted session `seed-ratio` now restores the typed default `1.0` without expanding the compact wire map; `aria2-core` package version `0.3.4` is synchronized in its manifest and lockfile; in-memory BT and Metalink post-download matching now retains the source URI and recognizes URI extensions when no output file exists; current core test and all-features Clippy gates revalidated in the worktree |
| Next action | Continue remaining phase-2 RequestGroup force-halt, storage, checksum, paused session graph restart, and cross-protocol error/retry combinations; close the remaining TLS unsupported certificate representations and broader HTTPS interoperability matrix; retain the DHT local-node guard and actual announce-port propagation while separately closing public DHT interoperability |
 | Acceptance evidence | The checkpoints below record the focused session serializer, control-file/piece-length, checksum, progress-manager, Metalink dependency-removal, Metalink direct mirror and torrent metaurl retry, lifecycle, cancellation-safe frame, and real HTTP/FTP/SFTP shutdown-halt tests, reserved-queue fairness regression, HTTP response classification tests, HTTP, FTP, SFTP, and BitTorrent lifecycle/integrity/seeding E2E, including the production SFTP retry-wait pause test (`26 passed, 0 failed, 2 ignored`), the FTP control-connection total-attempt test (`1 passed, 0 failed`), the SFTP not-found total-attempt test (`1 passed, 0 failed`), the current `aria2-core` library result (`3518 passed, 0 failed, 1 ignored`) and the source-URI-aware in-memory torrent regression and the stateful in-memory HTTP 504 retry E2E (`1 passed, 0 failed`), RequestGroupMan lifecycle coverage (`41 passed, 0 failed`), terminal `DownloadResult`/`force-save` serializer and standard App shutdown/restart coverage, the standard Metalink graph piece-state restart regression (`1 passed, 0 failed`), the paused graph restart lifecycle regression and App test group (`24 passed, 0 failed`), the new real App Metalink graph restart execution regression (`1 passed, 0 failed`), FTP/SFTP retry-wait remove regressions and full protocol targets (`43`/`30` passed, 2 ignored), the current RPC library result (`233 passed, 0 failed`), the current all-features `aria2-protocol` library result (`836 passed, 0 failed`), the workspace regression command, workspace all-targets/all-features Clippy with `-D warnings`, full and targeted formatting, and diff checks; Node.js binding verification (`123 passed`), Python SDK verification (`137 passed`), broader force-halt/session graph variants, cross-protocol error combinations, measured performance, and final workspace acceptance remain open |
...
The Rust-owned empty-password PKCS#12 identity adapter retains the existing
`p12_q3` path for supported archives and falls back to `pkcs5` PBES2 decoding
for AES-128-CBC and AES-192-CBC. The fallback also walks encrypted safe
contents when `p12_q3` cannot decrypt the content and accepts a plaintext
`keyBag` containing PKCS#8 private-key DER. No option name, default, session
shape, RPC/CLI wire behavior, certificate chain handling, or product version
changed. The test fixtures are built inside the Rust test with corrected
PBKDF2 AlgorithmIdentifier encoding and do not depend on an external client.
...
AES-GCM, unsupported bag variants, password-bearing archives, broader
original-client/third-party HTTPS interoperability, and final workspace
acceptance remain open. The active phase remains
`phase-2-core-domain` (`in_progress`) and the migration remains `PARTIAL`.

### 2026-08-18 PKCS#12 PBKDF2 PRF compatibility checkpoint

The Rust-owned PBES2 adapter now preserves and decodes the standard PBKDF2
HMAC SHA-224, SHA-384, and SHA-512 PRF AlgorithmIdentifier values in addition
to the existing SHA-1 and SHA-256 paths. The adapter requires the standard
DER NULL PRF parameters before passing the algorithm to `pkcs5`; no new
password handling, option, session shape, RPC/CLI wire behavior, certificate
chain handling, or product version was introduced.
...
variants, password-bearing archives, broader original-client/third-party HTTPS
...
The `aria2-core` package version is `0.3.2`, with the four Rust workspace
members, Cargo lockfile, Python SDK, Node.js SDK, examples, README, and
distribution helper metadata synchronized to the current release identity.
The Node.js HTTP E2E fixture now gives each download scenario an isolated
output name, and the Python Windows E2E fixture discards child logs instead of
letting an unread subprocess pipe block concurrent RPC requests.

Rust-owned and SDK verification:

~~~text
Node.js: TypeScript typecheck PASS; Vitest 13 files, 123 passed
Python: pytest 137 passed in 38.93s
aria2-core all-features library: 3518 passed, 0 failed, 1 ignored
~~~

The migration remains `PARTIAL`; broader phase-2 protocol/lifecycle coverage,
...
cargo test -p aria2-core --all-features --lib -- --test-threads=1
  3518 passed, 0 failed, 1 ignored
...
`max_attempts` as the total number of requests for a piece. A value of `1`
therefore sends no retry, while a value of `3` permits exactly two retries.
Peer-level metadata exchange already used the same total-attempt semantics;
this closes the piece-level off-by-one gap without changing CLI, configuration,
session, or RPC wire shapes. The `aria2-core` package remains at version
`0.3.2`.
...
## 2026-08-18 In-Memory HTTP Metadata Retry Classification Checkpoint

The in-memory metadata pre-download path now uses the original HTTP
skip-response retry boundary while retaining the shared total-attempt policy:
504 Gateway Timeout is retryable, 502/503 are retryable only when
`retry-wait > 0`, 404 still follows `max-file-not-found`, and transient
network/timeout failures retry within `max-tries`. HTTP 500 and other 4xx/5xx
responses remain terminal metadata errors.
...
cargo test -p aria2-core --all-features --lib engine::download_command -- --test-threads=1
  29 passed, 0 failed
cargo test -p aria2-core --all-features --test deep_e2e_bittorrent follow_torrent_mem_http_retries_gateway_timeout -- --exact --test-threads=1 --nocapture
  1 passed, 0 failed
cargo test -p aria2-core --all-features --test deep_e2e_bittorrent -- --test-threads=1
  34 passed, 0 failed, 2 ignored
cargo test -p aria2-core --all-features --lib -- --test-threads=1
  3507 passed, 0 failed, 1 ignored
cargo clippy -p aria2-core --all-targets --all-features -- -D warnings
  PASS
cargo fmt --all -- --check
  PASS
...
This closes only the in-memory HTTP metadata retry-classification slice.
Broader cross-protocol error combinations, original-client and third-party
interoperability, bindings, measured performance, and final workspace
acceptance remain open; the active phase stays `phase-2-core-domain` and
...
The Rust-owned BitTorrent engine now has production-path coverage for a
multi-file torrent using `prealloc` before peer execution. The test sends
`ForceHaltAll` after the peer path is active and verifies that both allocated
payload files retain their layout and requested lengths while the group keeps
an in-progress result. The allocation-waiter cancellation boundary remains
covered separately by the engine-loop regression, which exercises a queued
allocation and verifies that force-halt wakes its owner.
...
## 2026-08-18 In-Memory Metadata Source URI Checkpoint

In-memory metadata downloads do not create a DownloadContext or output file.
Post-download matching now populates the existing
CompletedDownloadInfo.base_uri with the effective source URI and uses its
parsed URI path for BitTorrent and Metalink extension criteria. Existing
content-type and local-file matching APIs remain unchanged.

Rust-owned verification in this worktree:

~~~text
cargo test -p aria2-core --all-features --lib engine::post_download_handler -- --test-threads=1
  4 passed, 0 failed
cargo test -p aria2-core --all-features --lib engine::bt_torrent_post_download_handler -- --test-threads=1
  4 passed, 0 failed
cargo test -p aria2-core --all-features --lib engine::metalink_post_download_handler -- --test-threads=1
  6 passed, 0 failed
cargo test -p aria2-core --all-features --test deep_e2e_bittorrent follow_torrent_mem_http_uses_source_uri_extension -- --test-threads=1
  1 passed, 0 failed
cargo test -p aria2-core --all-features --lib -- --test-threads=1
  3498 passed, 0 failed, 1 ignored
cargo clippy -p aria2-core --all-targets --all-features -- -D warnings
  PASS
cargo fmt --all -- --check
  PASS
...
This closes the source-URI detection gap for memory-backed metadata
post-download processing. The active phase remains
phase-2-core-domain (in_progress) and the migration remains PARTIAL; broader
cross-protocol lifecycle combinations, interoperability, bindings, measured
...
This closes only the BitTorrent no-progress timeout slice. BitTorrent remains
`PARTIAL` until broader scheduler and seeding parity, dependency coverage,
original-client and third-party interoperability, bindings, measured
performance evidence, and final workspace acceptance have reproducible
evidence.
...
payload BitTorrent slice. BitTorrent remains `PARTIAL` until broader scheduler
and seeding parity, dependency coverage, original-client and third-party
interoperability, bindings, measured performance evidence, and final workspace
acceptance have reproducible evidence.
...
`aria2c --version` value of `aria2-rust 0.3.1`, and
`cargo clippy -p aria2 --all-targets --all-features -- -D warnings`.
...
3,453 passed and 1 ignored, and every integration, E2E, stress, and performance
target in that command passed. The aggregate workspace command
`cargo test --workspace --all-features -j 1 --quiet -- --test-threads=1` also
completed with exit code 0 on 2026-08-17; ignored tests remain explicitly
reported and are not counted as passing evidence.
...
The post-change core all-features library aggregate now reports `3518 passed,
0 failed, 1 ignored`; the active phase remains `phase-2-core-domain` and the
migration remains `PARTIAL`.

### Metalink `max-file-not-found` mirror checkpoint (2026-08-18)

Metalink direct-mirror failover now consults the owning `RequestGroup` after
recording an HTTP 404. With `max-file-not-found=0`, the first 404 remains the
public `ResourceNotFound` result and stops the mirror loop; positive limits
continue to permit failover until the configured count reaches
`MaxFileNotFound`. The same group-aware stop condition is applied to torrent
metaurl fallback loops. This preserves the existing `aria2-core` package
version `0.3.2` and changes no CLI, configuration, session, or RPC wire
shape.

Rust-owned verification in this worktree:

~~~text
cargo test -p aria2-core --all-features --lib metalink_download_command -- --test-threads=1
  22 passed, 0 failed
cargo test -p aria2-core --all-features --lib -- --test-threads=1
  3499 passed, 0 failed, 1 ignored
cargo clippy -p aria2-core --all-targets --all-features -- -D warnings
  PASS
cargo fmt --all -- --check
  PASS
...
This closes only the Metalink 404 mirror-limit slice. Broader HTTP range/gap
status combinations, cross-protocol retry semantics, interoperability,
bindings, measured performance, and final workspace acceptance remain open;
the active phase remains `phase-2-core-domain` and the migration remains
`PARTIAL`.

### SFTP transient retry classification checkpoint (2026-08-18)

SFTP command execution now applies the shared `RetryPolicy` to temporary
network and timeout failures. Remote not-found responses retain their separate
`max-file-not-found` counter, while authentication and other terminal errors
remain non-retryable. The policy uses total-attempt semantics, including
`max-tries=0` as unlimited.
...
cargo test -p aria2-core --all-features --test test_e2e_sftp_download -- --test-threads=1
  30 passed, 0 failed, 2 ignored
cargo test -p aria2-core --all-features --lib -- --test-threads=1
  3501 passed, 0 failed, 1 ignored
cargo clippy -p aria2-core --all-targets --all-features -- -D warnings
  PASS
cargo fmt --all -- --check
  PASS
...
This closes only the SFTP transient-retry classification slice. Broader HTTP
range/gap status combinations, cross-protocol lifecycle combinations,
interoperability, bindings, measured performance, and final workspace
acceptance remain open; the active phase remains `phase-2-core-domain` and the
migration remains `PARTIAL`.

### HTTP 408/429 default retry classification checkpoint (2026-08-18)

The default `RetryPolicy` classification now maps HTTP 408 (Request Timeout)
and 429 (Too Many Requests) to `RecoverableError::ServerError`, so sequential
HTTP, HTTP range/gap, Metalink HTTP, and authentication-retry paths can retry
these transient responses. The existing special handling for 404, 401/407,
and 416 remains unchanged; the standalone `http/skip_response` compatibility
rule for 500/502/503/504 and `retry-wait` is also unchanged.

Rust-owned verification in this worktree:

~~~text
cargo test -p aria2-core --all-features --lib sequential_download -- --test-threads=1
  32 passed, 0 failed
cargo test -p aria2-core --all-features --lib http_segment_downloader -- --test-threads=1
  19 passed, 0 failed
cargo test -p aria2-core --all-features --lib metalink_download_command -- --test-threads=1
  6 passed, 0 failed
cargo test -p aria2-core --all-features --lib http::skip_response -- --test-threads=1
  36 passed, 0 failed
cargo test -p aria2-core --all-features --lib -- --test-threads=1
  3506 passed, 0 failed, 1 ignored
cargo clippy -p aria2-core --all-targets --all-features -- -D warnings
  PASS
cargo fmt --all -- --check
  PASS
...
This closes only the default HTTP 408/429 retry-classification slice. Broader
cross-protocol error combinations, original-client interoperability, bindings,
measured performance, and final workspace acceptance remain open; the active
phase remains `phase-2-core-domain` and the migration remains `PARTIAL`.

### Concurrent HTTP range retry and mirror failover checkpoint (2026-08-18)

The concurrent HTTP range pipeline now uses the shared retry boundary for each
segment. HTTP 500 and other terminal 4xx/5xx responses stop the current URI
without spending the replacement mirror's retry budget; HTTP 504 remains
retryable; HTTP 502/503 remain retryable only when `retry-wait` permits the
retry; and HTTP 404 continues through the owning `RequestGroup`'s
`max-file-not-found` counter. When a terminal segment failure has another
available URI, the segment is reassigned to that mirror with a fresh retry
count, matching aria2's terminal-URI failover behavior. Capacity-limited 429
and 503 responses retain the adaptive requeue path.
...
This closes only the concurrent HTTP range terminal-classification and
terminal-mirror-failover slice. Broader cross-protocol error combinations,
original-client and third-party interoperability, bindings, measured
performance, and final workspace acceptance remain open; the active phase
remains `phase-2-core-domain` and the migration remains `PARTIAL`.
