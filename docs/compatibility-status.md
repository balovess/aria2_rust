...
| Latest verification | 2026-08-28 current-host refresh: `cargo test -j 1 -p aria2-core --all-features --lib http::client_identity -- --test-threads=1` reports `22 passed, 0 failed`, including Rust-owned AES-128-GCM/AES-256-GCM empty-password PFX identity and unknown-additional-bag coverage; current Cargo manifest, lockfile, and metadata report `aria2-core` `0.3.3`; historical focused results remain retained below. |
| Latest current-host crate test refresh | On the current worktree, `aria2-core` reports `3596 passed, 0 failed, 1 ignored`; `aria2-protocol` library/integration targets report `840 + 6 + 53 passed`; `aria2-rpc` library/integration/E2E targets report `141 + 9 + 55 + 2 + 47 + 5 + 4 + 31 + 5 + 2 + 7 + 5 passed`; and `aria2` library/binary/integration/regression targets report `334 passed, 0 failed, 3 ignored`. The corrected protocol `--lib --tests` command passes; the prior `--all-targets` form is unsuitable with forwarded test-thread arguments because Criterion benches reject them. Workspace all-targets/all-features Clippy with `-D warnings` passes; full `cargo fmt --all -- --check` and `git diff --check` pass. |
