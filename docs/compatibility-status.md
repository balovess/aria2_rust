...
| Latest focused cross-protocol verification | Rust-owned `deep_e2e_cross_protocol` reports `18 passed, 0 failed, 2 ignored` for Metalink checksum/mirror behavior, FTP authentication, rate limiting, disk-write failure propagation, session restore, filename collision, and local DHT/LPD fixtures; no production behavior changed in this refresh; broader protocol error/retry combinations, live interoperability, measured performance, and final workspace acceptance remain open |
