...
| 7 | Torrent metaurl handling | P1 | FIXED | metaurl-only files are preserved by `MetalinkToRequestGroup`; `create_multi_file_for_single()` carries torrent metaurls into the executable `BtDependency` fallback (bittorrent-gated) |
...
| 7 | Metalink | Torrent metaurl handling | FIXED | `MetalinkToRequestGroup` no longer drops metaurl-only files; `MetalinkDownloadCommand` accepts them and falls back to downloading the `.torrent` + running `BtDownloadCommand` when no HTTP mirror succeeds (by priority, `metalink_download_command/execution.rs::try_torrent_metaurl`, gated on `bittorrent` feature). `FileDownloadInfo.torrent_metaurls` is populated in both single and multi-file modes |
