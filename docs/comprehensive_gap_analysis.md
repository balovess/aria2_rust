...
|-----------|----------|-----|
| BtSetup | P1 | Missing orchestrator that wires all per-download BT components |
| DefaultBtMessageFactory | P1 | Context injection (PeerStorage on Interested/Piece, DHT on Port) |
| Zero-copy Piece optimization | P2 | Currently copies Piece data; C++ uses zero-copy path |
| `addAllowedFastMessageToQueue()` | P2 | Always empty with TODO |
...
| Sub-module | Status | Key Gaps |
|------------|--------|----------|
| FtpConnection | Complete | Commands, connector, FEAT, parser, transfer |
| FtpNegotiation | Complete | MDTM, PWD/CWD, FEAT, SIZE, REST all supported |
...
| # | Issue | Priority | Status | Details |
|---|-------|----------|--------|---------|
| 3 | `BtFileAllocationEntry` missing | P1 | TODO | Skip already-allocated pieces via PieceStorage::getBitfield() |
| 4 | `HttpFileAllocationEntry` missing | P1 | TODO | HTTP downloads with disk cache integration |
| 5 | Allocation progress events | P2 | TODO | C++ `FileAllocationMan::getProgress()` |
...
| Component | Status | Notes |
|-----------|--------|-------|
| `--log-max-size` / `--log-max-files` | Missing | Log rotation |
| `trace` log level | Missing | New level below `debug` |
| Rotating file sink | Missing | BoundedFormatter |
...
| `--max-connection-per-server` max | 16 | 1024 | NOT Adopted |
...
| Item | Status |
|------|--------|
| `reduceActiveDownloadsToLimit()` | Missing |
| Peer rename (`bad peer` -> `temporarily rejected peer`) | NOT Adopted |
| `--detach-share-only` rename | NOT Adopted |
...
| 19 | CheckIntegrity | CheckIntegrityMan queue missing | FIXED | `checksum/check_integrity/man.rs` adds the queue + background worker (mirrors C++ CheckIntegrityMan + CheckIntegrityDispatcherCommand + CheckIntegrityCommand): chunked `validate_chunk` with `yield_now` between chunks, sequential by default, `oneshot` outcome notification, cancellation. New `FileChunkValidator` reads the file chunk-by-chunk against expected piece digests (short reads → mismatch, not I/O error). Wired into BT (`BtDownloadCommand` single-file) and HTTP (`DownloadCommand`, when the context has piece hashes e.g. Metalink); `DownloadOptions.check_integrity` added and parsed from `--check-integrity` in apply.rs + RPC + session. 5 tests |
| 20 | aria2-next | spdlog rotating logs | Missing |
...
| 24 | aria2-next | Control file removal on user halt | DONE | `sequential_download/download_flow.rs` + `file_lifecycle.rs` remove control file on completion |
...
7. ~~Metalink chunk-level piece hash + metaurl handling~~ — FIXED 2026-07-31 (`verify_pieces` + BtDependency fallback)
...
12. aria2-next: spdlog rotating logs — Missing
