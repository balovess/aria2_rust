...
use aria2_core::config::{OptionRegistry, OptionValue, OptionType, OptionDef};
...
fn bench_option_def_validation(c: &mut Criterion) {
    ...
    c.bench_with_input(BenchmarkId::new("option_validate_100_ints", 100), &test_vals, |b, vals| {
        b.iter(|| {
            for v in vals.iter() {
...
fn bench_option_value_display(c: &mut Criterion) {
    ...
    c.bench_with_input(BenchmarkId::new("optionvalue_display_5_types", 5), &values, |b, vals| {
        b.iter(|| {
            for v in vals.iter() {
                let s = format!("{}", v);
                black_box(s);
...
fn bench_format_size_inline(c: &mut Criterion) {
    ...
    c.bench_with_input(BenchmarkId::new("format_size_inline_100", 100), &values, |b, vals| {
        b.iter(|| {
            for v in vals.iter() {
                let s = if *v < 1024 { format!("{}B", v) }
                    else if *v < 1024 * 1024 { format!("{:.1}KiB", *v as f64 / 1024.0) }
                    else if *v < 1024 * 1024 * 1024 { format!("{:.1}MiB", *v as f64 / (1024.0 * 1024.0)) }
                    else { format!("{:.1}GiB", *v as f64 / (1024.0 * 1024.0 * 1024.0)) };
                black_box(s);
            }
        });
    });
...
fn bench_format_speed_inline(c: &mut Criterion) {
    ...
    c.bench_with_input(BenchmarkId::new("format_speed_inline_100", 100), &speeds, |b, spds| {
        b.iter(|| {
            for s in spds.iter() {
                let f = if *s < 1024 { format!("{}B/s", s) }
                    else if *s < 1024 * 1024 { format!("{:.1}KiB/s", *s as f64 / 1024.0) }
                    else if *s < 1024 * 1024 * 1024 { format!("{:.1}MiB/s", *s as f64 / (1024.0 * 1024.0)) }
                    else { format!("{:.1}GiB/s", *s as f64 / (1024.0 * 1024.0 * 1024.0)) };
                black_box(f);
            }
        });
    });
...
fn bench_parse_cli_args(c: &mut Criterion) {
    ...
    let args: Vec<&str> = vec![
        "--dir=/tmp", "--split=8", "--max-tries=5", "--timeout=60",
        "--quiet=true", "--max-download-limit=0", "--check-certificate=true",
    ];
    let registry = gen_registry();
    c.bench_with_input(BenchmarkId::new("parse_cli_args_7_args", 7), &args, |b, args| {
        b.iter(|| {
