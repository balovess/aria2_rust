...
use aria2_core::request::request_group::{
    DownloadOptions, DownloadResultCode, DownloadStatus, FollowMode, GroupId, HaltReason,
    RequestGroup,
};
...
#[tokio::test]
async fn test_e2e_engine_sequential_http_shutdown_halt_preserves_resume_state() {
    let server = start_server().await;
    let dir = tmp_dir();
    let output_name = "engine-sequential-shutdown-halt.bin";
    let output_path = dir.path().join(output_name);
    let control_path = ControlFile::control_path_for(&output_path);
    let url = format!("{}/files/slow_stream_test.bin", server.base_url());
    let options = DownloadOptions {
    ...
    tokio::time::timeout(std::time::Duration::from_secs(10), async {
        loop {
            if output_path.exists()
                && ControlFile::load(&control_path)
                    .await
                    .ok()
                    .flatten()
                    .is_some_and(|checkpoint| checkpoint.completed_length() > 0)
            {
                return;
            }
            tokio::time::sleep(std::time::Duration::from_millis(10)).await;
    ...
    .await
    .expect("HTTP shutdown halt test did not make resumable progress");

    command_tx
        .send(EngineCommand::HaltAll {
            reason: HaltReason::ShutdownSignal,
        })
        .expect("HTTP shutdown halt command should be accepted");
    wait_for_http_engine(engine_task, "HTTP shutdown halt did not stop the engine").await;
    ...
    let checkpoint = ControlFile::load(&control_path)
        .await
        .expect("HTTP shutdown halt control file should be readable")
        .expect("HTTP shutdown halt must preserve its control file");
    assert!(checkpoint.completed_length() > 0);
    assert!(checkpoint.completed_length() < checkpoint.total_length());
}

#[tokio::test]
async fn test_e2e_engine_sequential_http_force_halt_preserves_resume_state() {
    let server = start_server().await;
    let dir = tmp_dir();
    let output_name = "engine-sequential-force-halt.bin";
    let output_path = dir.path().join(output_name);
    let control_path = ControlFile::control_path_for(&output_path);
    let url = format!("{}/files/slow_stream_test.bin", server.base_url());
    let options = DownloadOptions {
    ...
    .await
    .expect("HTTP force halt test did not make resumable progress");

    command_tx
        .send(EngineCommand::ForceHaltAll {
            reason: HaltReason::ShutdownSignal,
        })
        .expect("HTTP force halt command should be accepted");
    wait_for_http_engine(engine_task, "HTTP force halt did not stop the engine").await;
    ...
    let checkpoint = ControlFile::load(&control_path)
        .await
        .expect("HTTP force halt control file should be readable")
        .expect("HTTP force halt must preserve its control file");
    assert!(checkpoint.completed_length() > 0);
    assert!(checkpoint.completed_length() < checkpoint.total_length());
}
...
#[tokio::test]
async fn test_e2e_engine_sequential_http_force_remove_preserves_control_file() {
    let server = start_server().await;
    let dir = tmp_dir();
    let output_name = "engine-sequential-force-remove.bin";
    let output_path = dir.path().join(output_name);
    let control_path =
        aria2_core::filesystem::control_file::ControlFile::control_path_for(&output_path);
    let url = format!("{}/files/slow_stream_test.bin", server.base_url());
    ...
    .await
    .expect("sequential HTTP force-remove test did not make progress");

    command_tx
        .send(EngineCommand::ForceRemoveDownload { gid })
        .expect("HTTP force-remove command should be accepted");
    wait_for_http_engine(
        engine_task,
        "sequential HTTP force-remove did not stop promptly",
    )
    .await;
    ...
    assert!(
        output_path.exists(),
        "force-remove must retain partial sequential HTTP output"
    );
    assert!(
        control_path.exists(),
        "force-remove must preserve the sequential HTTP control file"
    );
}
