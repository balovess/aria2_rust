...
async fn test_no_data_loss_partial_write() {
    ...
    assert_eq!(
        &content[500..700],
        &additional_data[..],
        "Additional data should be written"
    );
}
...
async fn test_data_integrity_random_access() {
    ...
    let segments = [(0, b"SEG0"), (500, b"SEG5"), (100, b"SEG1"), (900, b"SEG9")];
...
async fn test_resume_helper_no_continue() {
    ...
    assert!(
        !state.should_resume,
        "Should not resume without continue flag"
    );
...
async fn test_full_recovery_cycle() {
    ...
    let content = tokio::fs::read(&path).await.unwrap();
    assert!(
        content.starts_with(b"phase1"),
        "Phase1 data should be intact"
    );

    // Load control file and verify
    let loaded_cf = ControlFile::load(&ctrl_path).await.unwrap().unwrap();
    assert!(
        loaded_cf.is_piece_done(0),
        "Piece 0 should still be marked done"
    );
}
