...
async fn test_connection_reset_error_handling() {
    ...
    if result.is_ok() {
        ...
        if output_path.exists() {
            ...
            assert!(content.len() < 1000000, "File should be smaller than claimed size");
        }
    }
...
async fn test_dns_resolution_invalid_hostname() {
    ...
    if result.is_err() {
        ...
        assert!(retry_result.is_err(), "Retry should be blocked by negative cache");
...
async fn test_dns_ipv4_preference_on_resolution() {
    ...
    if let Ok(addrs) = result {
        ...
        let has_ipv4 = addrs.iter().any(|a| matches!(a.ip(), std::net::IpAddr::V4(_)));
        let has_ipv6 = addrs.iter().any(|a| matches!(a.ip(), std::net::IpAddr::V6(_)));
        
        if has_ipv4 && has_ipv6 {
            let first_ipv4_pos = addrs.iter().position(|a| matches!(a.ip(), std::net::IpAddr::V4(_)));
            let first_ipv6_pos = addrs.iter().position(|a| matches!(a.ip(), std::net::IpAddr::V6(_)));
...
async fn test_max_connections_limit_error() {
    ...
    match result {
        ...
        Err(e) => {
            ...
            let err_str = e.to_string();
            assert!(err_str.contains("limit") 
                   || err_str.contains("max")
                   || err_str.contains("最大")
                   || err_str.contains("限制"),
                   "Error should indicate connection limit: {}", e);
        }
...
async fn test_connection_cleanup_on_error() {
    ...
    match result {
        Err(_) => {
            ...
            assert_eq!(manager.pool_size(), 0, "No connections should remain after failed acquire");
...
fn test_redirect_loop_detection() {
    ...
    assert!(err.to_string().contains("loop") || err.to_string().contains("循环"),
           "Error should mention loop: {}", err);
}
...
fn test_max_redirects_exceeded() {
    ...
    let err = result.unwrap_err();
    assert!(err.to_string().contains("max") || err.to_string().contains("最大"),
           "Error should mention max redirects: {}", err);
}
...
fn test_redirect_missing_location_header() {
    ...
    let err = result.unwrap_err();
    assert!(err.to_string().contains("Location") || err.to_string().contains("缺少"),
           "Error should mention missing Location: {}", err);
}
...
fn test_retry_policy_recoverable_vs_fatal() {
    ...
    assert!(policy.should_retry(0, &Aria2Error::Recoverable(RecoverableError::TemporaryNetworkFailure { message: "test".into() })));
    
    // Fatal errors should NOT allow retry
    assert!(!policy.should_retry(0, &Aria2Error::Fatal(FatalError::DiskSpaceExhausted)));
    assert!(!policy.should_retry(0, &Aria2Error::Fatal(FatalError::PermissionDenied { path: "/test".into() })));
...
fn test_retry_stats_categories() {
    ...
    stats.record_retry(&Aria2Error::Recoverable(RecoverableError::ServerError { code: 503 }));
    stats.record_retry(&Aria2Error::Recoverable(RecoverableError::TemporaryNetworkFailure { message: "conn reset".into() }));
    stats.record_retry(&Aria2Error::Recoverable(RecoverableError::MaxTriesReached { attempts: 5 }));
...
fn test_error_display_formatting() {
    ...
    assert!(timeout_err.to_string().contains("timeout") || timeout_err.to_string().contains("Timeout"));
    ...
    let perm_err = Aria2Error::Fatal(FatalError::PermissionDenied { path: "/root/test".into() });
    assert!(perm_err.to_string().contains("Permission") || perm_err.to_string().contains("/root/test"));
}
