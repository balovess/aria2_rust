...
impl RawFtpControl {
    async fn connect(host: &str, port: u16) -> Result<Self> {
        ...
        let stream = tokio::net::TcpStream::connect(socket_addr)
            ...
            .map_err(|e| {
                Aria2Error::Recoverable(RecoverableError::TemporaryNetworkFailure {
                    message: format!("FTP connect failed: {}", e),
                })
            })?;
    ...
    async fn read_response(&mut self, timeout: Duration) -> Result<(u16, String)> {
        ...
        loop {
            ...
            let bytes_read = tokio::time::timeout(timeout, self.reader.read_line(&mut line))
                ...
                .map_err(|_| {
                    Aria2Error::Recoverable(RecoverableError::TemporaryNetworkFailure {
                        message: "FTP response timeout".into(),
                    })
                })?
                .map_err(|e| {
                    Aria2Error::Recoverable(RecoverableError::TemporaryNetworkFailure {
...
impl Command for FtpDownloadCommand {
    async fn execute(&mut self) -> Result<()> {
        ...
        ctrl.command(&format!("USER {}", self.username))
            .await
            .map_err(|e| {
                Aria2Error::Fatal(FatalError::Config(format!("FTP USER failed: {}", e)))
            })?;
        ...
        let pass_resp = ctrl
            ...
            .await
            .map_err(|e| {
                Aria2Error::Fatal(FatalError::Config(format!("FTP PASS failed: {}", e)))
            })?;
        ...
        ctrl.command("TYPE I").await.map_err(|e| {
            Aria2Error::Recoverable(RecoverableError::TemporaryNetworkFailure {
                message: format!("TYPE I failed: {}", e),
            })
        })?;

        let size_resp = ctrl
            .command(&format!("SIZE {}", self.remote_path))
            .await
            .unwrap_or((0, "0".into()));
        ...
        let pasv_resp = ctrl.command("PASV").await.map_err(|e| {
            Aria2Error::Recoverable(RecoverableError::TemporaryNetworkFailure {
                message: format!("PASV failed: {}", e),
            })
        })?;

        let (data_host, data_port) = parse_pasv(&pasv_resp.1).ok_or_else(|| {
            Aria2Error::Recoverable(RecoverableError::TemporaryNetworkFailure {
                message: "cannot parse PASV".into(),
            })
        })?;

        let retr_resp = ctrl
            .command(&format!("RETR {}", self.remote_path))
            .await
            .map_err(|e| {
                let msg = format!("{}", e);
        ...
        if retr_resp.0 != 150 && retr_resp.0 != 125 {
            return Err(Aria2Error::Recoverable(
                RecoverableError::TemporaryNetworkFailure {
                    message: format!("RETR unexpected: {} {}", retr_resp.0, retr_resp.1),
                },
            ));
        }
        ...
        .map_err(|_| {
            Aria2Error::Recoverable(RecoverableError::TemporaryNetworkFailure {
                message: "data conn timeout".into(),
            })
        })?
        .map_err(|e| {
            Aria2Error::Recoverable(RecoverableError::TemporaryNetworkFailure {
                message: format!("data conn failed: {}", e),
            })
        })?;
        ...
        loop {
            let bytes_read = data_stream.read(&mut buffer).await.map_err(|e| {
                Aria2Error::Recoverable(RecoverableError::TemporaryNetworkFailure {
                    message: format!("read data error: {}", e),
                })
            })?;
