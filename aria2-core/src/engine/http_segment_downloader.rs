...
impl HttpSegmentDownloader {
    ...
    pub async fn supports_range(&self, url: &str) -> Result<bool> {
        let resp = self.client
            ...
            .send()
            .await
            .map_err(|e| Aria2Error::Recoverable(RecoverableError::TemporaryNetworkFailure { message: format!("HEAD request failed: {}", e) }))?;
    ...
    pub async fn download_range(
        ...
        let response = self.client
            ...
            .send()
            .await
            .map_err(|e| Aria2Error::Recoverable(RecoverableError::TemporaryNetworkFailure { message: format!("HTTP Range request failed: {}", e) }))?;
        ...
        match status.as_u16() {
            ...
            416 => {
                return Err(Aria2Error::Recoverable(RecoverableError::TemporaryNetworkFailure {
                    message: format!("Range not satisfiable: bytes={}-{}", offset, offset + length.saturating_sub(1)),
                }));
            }
            code if code >= 400 && code < 500 => {
                return Err(Aria2Error::Fatal(crate::error::FatalError::Config(format!("HTTP client error {}: {}", code, url))));
            }
            code if code >= 500 => {
        ...
        while let Some(chunk_result) = stream.next().await {
            match chunk_result {
                ...
                Err(e) => return Err(Aria2Error::Recoverable(RecoverableError::TemporaryNetworkFailure { message: format!("Stream read error: {}", e) })),
            }
        }

        if data.is_empty() && length > 0 {
            return Err(Aria2Error::Recoverable(RecoverableError::TemporaryNetworkFailure {
                message: format!("Empty response for range {}-{} from {}", offset, offset + length.saturating_sub(1), url),
            }));
        }
...
mod tests {
    ...
    async fn test_supports_range_no_server() {
        ...
        let result = dl.supports_range("http://127.0.0.1:1/nonexistent").await;
    ...
    async fn test_download_range_with_mock_http_416() {
        ...
        let result = dl.download_range(&url, 99999, 100).await;
    ...
    async fn test_supports_range_header_parsing() {
        ...
        assert!(dl.supports_range("http://invalid-host-name-that-does-not-exist-12345.com/").await.is_err());
    }
    ...
    async fn test_download_range_status_code_handling() {
        ...
        let result_404 = dl.download_range("http://httpbin.org/status/404", 0, 100).await;
