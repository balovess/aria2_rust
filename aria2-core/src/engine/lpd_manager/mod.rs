...
//! ```text
//! lpd_manager/
//!   ├── mod.rs          - LpdPeer, LpdManager, constants, public API
//!   ├── discovery.rs    - Peer discovery and message parsing
//!   └── multicast.rs    - LpdAnnouncer (UDP multicast I/O)
//!
//! LPD Protocol (BEP 14):
...
pub use multicast::LpdAnnouncer;
...
pub struct LpdManager {
    ...
    announcer: Arc<multicast::LpdAnnouncer>,
...
impl LpdManager {
    ...
    pub fn new() -> Self {
        let announcer = multicast::LpdAnnouncer::new().unwrap_or_else(|_| {
            ...
            multicast::LpdAnnouncer::with_config(constants::LPD_DEFAULT_ANNOUNCE_INTERVAL_SECS)
    ...
    pub fn with_interval(announce_interval_secs: u64) -> Result<Self, String> {
        let announcer = multicast::LpdAnnouncer::with_config(announce_interval_secs)?;
