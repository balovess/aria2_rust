...
use crate::segment::piece_storage::types::StreamPieceSelectorKind;
...
impl PieceStorage for DefaultPieceStorage {
    ...
    fn mark_piece_missing(&mut self, index: usize) {
        if index < self.bfman.num_pieces() && self.bfman.has_piece(index) {
            ...
            tracing::trace!("mark_piece_missing: piece {} marked as missing", index);
        }
    }
    ...
    fn advertise_piece(&mut self, cuid: u64, index: usize) {
        let now_ms = std::time::SystemTime::now()
            ...
            .unwrap_or_default()
            .as_millis() as u64;
        let entry = crate::segment::piece_storage::types::HaveEntry {
