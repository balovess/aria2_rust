...
use aria2_core::config::{ConfigManager, ConfigParser, OptionValue, UriListFile, UriListEntry};
...
fn regression_config_file_with_comments() {
    ...
    assert_eq!(parser.get_str("dir").unwrap(), "/downloads");
    assert_eq!(parser.get_i64("split").unwrap(), 8);
    assert!(!parser.contains("#"), "Comments should not be parsed as options");
}
...
fn regression_config_file_path_values() {
    ...
    assert_eq!(parser.get_str("save-session").unwrap(), "/var/lib/aria2/session.txt");
}
...
fn regression_config_file_url_values() {
    ...
    assert_eq!(parser.get_str("http-proxy").unwrap(), "http://proxy.example.com:8080");
}
...
async fn regression_config_manager_save_session() {
    ...
    mgr.set_global_option("dir", OptionValue::Str("/downloads".into())).await.unwrap();
    
    let temp_dir = tempfile::tempdir().unwrap();
    let session_path = temp_dir.path().join("session.txt");
    
    mgr.save_session(session_path.to_str().unwrap()).await.unwrap();
...
async fn regression_config_manager_load_session() {
    ...
    mgr1.set_global_option("split", OptionValue::Int(12)).await.unwrap();
    mgr1.set_global_option("quiet", OptionValue::Bool(true)).await.unwrap();
    
    let temp_dir = tempfile::tempdir().unwrap();
    let session_path = temp_dir.path().join("session.txt");
    
    mgr1.save_session(session_path.to_str().unwrap()).await.unwrap();
    
    // Then load into a new manager
    let mut mgr2 = ConfigManager::new();
    mgr2.load_session(session_path.to_str().unwrap()).await.unwrap();
...
async fn regression_session_roundtrip() {
    ...
    mgr.set_global_option("dir", OptionValue::Str("/var/downloads".into())).await.unwrap();
    mgr.set_global_option("quiet", OptionValue::Bool(true)).await.unwrap();
    mgr.set_global_option("timeout", OptionValue::Int(300)).await.unwrap();
    ...
    assert_eq!(mgr2.get_global_str("dir").await, Some("/var/downloads".into()));
    assert_eq!(mgr2.get_global_bool("quiet").await, Some(true));
...
async fn regression_config_manager_load_cli() {
    ...
    mgr.load_cli(&["--split=8".to_string(), "--quiet".to_string()]).await;
...
fn regression_uri_list_file_basic() {
    ...
    assert_eq!(uri_list.entries()[0].uris[0], "http://example.com/file1.zip");
}
