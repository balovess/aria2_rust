...
fn assert_success(resp: &JsonRpcResponse) {
    assert!(resp.is_success(), "Expected success response, got error: {:?}", resp.error);
}
...
async fn regression_add_uri_returns_gid_format() {
    ...
    let req = make_request("aria2.addUri", serde_json::json!(["http://example.com/file.zip"]));
    ...
    assert!(gid.chars().all(|c| c.is_ascii_hexdigit()), "GID should be hexadecimal");
}
...
async fn regression_add_uri_with_options() {
    let engine = RpcEngine::new();
    let req = make_request("aria2.addUri", serde_json::json!([
...
async fn regression_tell_active_returns_array() {
    ...
    // Add multiple tasks
    for i in 0..3 {
        let req = make_request("aria2.addUri", serde_json::json!([format!("http://example.com/file{}", i)]));
...
async fn regression_global_stat_camel_case_fields() {
    ...
    assert!(stat.get("downloadSpeed").is_some(), "downloadSpeed field required");
    assert!(stat.get("uploadSpeed").is_some(), "uploadSpeed field required");
    assert!(stat.get("numActive").is_some(), "numActive field required");
    assert!(stat.get("numWaiting").is_some(), "numWaiting field required");
    assert!(stat.get("numStopped").is_some(), "numStopped field required");
}
...
async fn regression_get_global_option_returns_object() {
    ...
    assert_success(&resp);
    assert!(resp.result.unwrap().is_object(), "Should return JSON object");
}
...
async fn regression_change_global_option_returns_ok() {
    ...
    let req = make_request("aria2.changeGlobalOption", serde_json::json!([
...
async fn regression_change_option_validates_keys() {
    ...
    // Invalid option key
    let invalid_req = make_request("aria2.changeOption", serde_json::json!([gid, {"invalid-option": "value"}]));
...
async fn regression_get_peers_returns_array() {
    ...
    assert_success(&resp);
    assert!(resp.result.unwrap().is_array(), "Should return array of peers");
}
...
async fn regression_get_uris_format() {
    ...
    let add_req = make_request("aria2.addUri", serde_json::json!([
        ["http://example.com/file.zip", "http://mirror.example.com/file.zip"]
    ]));
    let add_resp = engine.handle_request(&add_req).await;
    let gid: String = serde_json::from_value(add_resp.result.unwrap()).unwrap();
...
async fn regression_get_files_format() {
    ...
    let add_req = make_request("aria2.addUri", serde_json::json!(["http://example.com/file.zip"]));
    let add_resp = engine.handle_request(&add_req).await;
    let gid: String = serde_json::from_value(add_resp.result.unwrap()).unwrap();
...
async fn regression_get_servers_format() {
    ...
    for entry in &servers {
        ...
        for server in servers_arr {
            ...
            assert!(server.get("download_speed").is_some() || server.get("downloadSpeed").is_some());
        }
    }
}
...
async fn regression_change_uri_modifies_list() {
    ...
    let add_req = make_request("aria2.addUri", serde_json::json!([
        ["http://example.com/file.zip", "http://mirror1.example.com/file.zip"]
    ]));
    let add_resp = engine.handle_request(&add_req).await;
    let gid: String = serde_json::from_value(add_resp.result.unwrap()).unwrap();
    
    // Remove first URI, add new one
    let req = make_request("aria2.changeUri", serde_json::json!([
        gid,
        0,  // file index
...
async fn regression_change_position_modifies_position() {
    ...
    let add_req = make_request("aria2.addUri", serde_json::json!([
        ["http://uri1.example.com/file", "http://uri2.example.com/file", "http://uri3.example.com/file"]
    ]));
    let add_resp = engine.handle_request(&add_req).await;
    let gid: String = serde_json::from_value(add_resp.result.unwrap()).unwrap();
...
async fn regression_get_version_format() {
    ...
    assert!(version.get("enabledFeatures").is_some(), "enabledFeatures field required");
    
    let features = version.get("enabledFeatures").unwrap().as_array().unwrap();
    assert!(!features.is_empty(), "Should have at least one enabled feature");
}
...
async fn regression_get_session_info_format() {
    ...
    assert!(session.get("sessionId").is_some(), "sessionId field required");
    assert!(!session.get("sessionId").unwrap().as_str().unwrap().is_empty());
}
...
async fn regression_save_session_format() {
    ...
    // Add a task
    let req = make_request("aria2.addUri", serde_json::json!(["http://example.com/file"]));
    engine.handle_request(&req).await;
...
async fn regression_multicall_executes_batch() {
    ...
    let req = make_request("system.multicall", serde_json::json!([
        [
...
async fn regression_remove_download_result_returns_ok() {
    ...
    let req = make_request("aria2.removeDownloadResult", serde_json::json!(["some-gid"]));
    let resp = engine.handle_request(&req).await;
...
async fn regression_list_notifications_returns_7() {
    ...
    assert_eq!(notifications.len(), 7, "Should return exactly 7 notifications");
