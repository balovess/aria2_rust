...
use aria2_rpc::websocket::{EventPublisher, DownloadEvent, NotificationBatcher};
    ...
    for i in 0..500 {
        ...
        handles.push(tokio::spawn(async move {
            ...
            let gid = if response.is_success() {
                response.result.and_then(|r| r.as_str().map(|s| s.to_string()))
            } else {
                None
            };
...
async fn test_stress_websocket_event_publisher() {
    ...
    for i in 0..500 {
        ...
        publish_handles.push(tokio::spawn(async move {
            let event = DownloadEvent::download_start(
                format!("gid-{}", i),
                vec![json!({"index": i})],
            );
...
async fn test_stress_rpc_auth_concurrent() {
    ...
    let engine = Arc::new(
        RpcEngine::new()
            .with_auth_middleware(RpcAuthMiddleware::new("stress-test-token")),
    );
    ...
    let valid_success = results
        ...
        .count();
    assert_eq!(valid_success, 200, "All valid token requests should succeed");
    ...
    let invalid_failed = results
        ...
        .count();
    assert_eq!(invalid_failed, 100, "All invalid token requests should fail");
    
    println!(
        "Auth stress test: {} valid succeeded, {} invalid rejected",
        valid_success,
        invalid_failed
    );
}
...
async fn test_stress_task_lifecycle_operations() {
    ...
    for (i, gid) in gids.iter().enumerate() {
        ...
        handles.push(tokio::spawn(async move {
            // Pause
            let pause_req = JsonRpcRequest::new("aria2.pause", json!([gid_clone.clone()])).with_id(i * 4);
            let pause_resp = engine_clone.handle_request(&pause_req).await;
            
            // Unpause
            let unpause_req = JsonRpcRequest::new("aria2.unpause", json!([gid_clone.clone()])).with_id(i * 4 + 1);
            let unpause_resp = engine_clone.handle_request(&unpause_req).await;
            
            // Tell status
            let status_req = JsonRpcRequest::new("aria2.tellStatus", json!([gid_clone.clone()])).with_id(i * 4 + 2);
            let status_resp = engine_clone.handle_request(&status_req).await;
            
            // Remove
            let remove_req = JsonRpcRequest::new("aria2.remove", json!([gid_clone.clone()])).with_id(i * 4 + 3);
...
async fn test_stress_concurrent_global_options() {
    ...
    let success_count = results
        ...
        .count();
    assert_eq!(success_count, 200, "All 200 option operations should succeed");
    
    println!("Global options stress test: {} operations completed", success_count);
}
...
async fn test_stress_websocket_subscriber_churn() {
    ...
    for i in 0..iterations {
        ...
        assert_eq!(count, 0, "Subscriber should be removed after churn iteration {}", i);
    }
    
    // Final verification
    let final_count = publisher.subscriber_count().await;
    assert_eq!(final_count, 0, "No subscribers should remain after churn test");
