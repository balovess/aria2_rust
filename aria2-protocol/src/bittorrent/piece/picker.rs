...
pub struct PiecePicker {
    ...
    #[allow(dead_code)]
    // Seed for deterministic piece selection; must remain to support future reproducible/deterministic piece picking (e.g., for testing or reproducible downloads)
    rng_seed: u64,
...
mod tests {
    ...
    fn test_sequential_performance_with_large_torrent() {
        ...
        assert_eq!(
            picked,
            Some(5000),
            "Should pick piece 5000 after completing 0-4999"
        );
    ...
    fn test_no_pieces_skipped_in_sequential_mode() {
        ...
        assert_eq!(
            picked_pieces.len(),
            90,
            "Should have picked 90 remaining pieces"
        );

        // Verify no completed pieces were picked
        for &completed in &completed_indices {
            assert!(
                !picked_pieces.contains(&completed),
                "Completed piece {} should not be picked",
                completed
            );
        }
        ...
        for i in 1..picked_pieces.len() {
            assert!(
                picked_pieces[i] > picked_pieces[i - 1],
                "Pieces should be picked in order"
            );
        }
    }
    ...
    fn test_sequential_head_no_pieces_skipped() {
        ...
        assert_eq!(
            picked_pieces.len(),
            66,
            "Should have picked 66 remaining pieces"
        );
        ...
        for i in 1..picked_pieces.len() {
            assert!(
                picked_pieces[i] > picked_pieces[i - 1],
                "Pieces should be in ascending order"
            );
        }
    }
    ...
    fn test_sequential_tail_no_pieces_skipped() {
        ...
        for i in 1..picked_pieces.len() {
            assert!(
                picked_pieces[i] < picked_pieces[i - 1],
                "Pieces should be in descending order"
            );
        }
    }
    ...
    fn test_export_bitfield_empty() {
        ...
        assert_eq!(
            bitfield,
            vec![0x00, 0x00],
            "Empty bitfield should be all zeros"
        );
    }
    ...
    fn test_import_bitfield() {
        ...
        assert!(
            picker.get_piece_info(0).unwrap().completed,
            "Piece 0 should be complete"
        );
        assert!(
            picker.get_piece_info(2).unwrap().completed,
            "Piece 2 should be complete"
        );
        assert!(
            picker.get_piece_info(5).unwrap().completed,
            "Piece 5 should be complete"
        );
        assert!(
            picker.get_piece_info(7).unwrap().completed,
            "Piece 7 should be complete"
        );

        assert!(
            !picker.get_piece_info(1).unwrap().completed,
            "Piece 1 should not be complete"
        );
        assert!(
            !picker.get_piece_info(3).unwrap().completed,
            "Piece 3 should not be complete"
        );
        assert!(
            !picker.get_piece_info(4).unwrap().completed,
            "Piece 4 should not be complete"
        );
        assert!(
            !picker.get_piece_info(6).unwrap().completed,
            "Piece 6 should not be complete"
        );

        assert_eq!(
            picker.completed_count(),
            4,
            "Should have 4 completed pieces"
        );
    }
    ...
    fn test_export_import_roundtrip() {
        ...
        for i in 0..24 {
            ...
            assert_eq!(
                p1, p2,
                "Piece {} completion should match after roundtrip",
                i
            );
        }
    ...
    fn test_export_bitfield_zero_pieces() {
        ...
        assert!(
            bitfield.is_empty(),
            "Zero pieces should produce empty bitfield"
        );
    }
}
