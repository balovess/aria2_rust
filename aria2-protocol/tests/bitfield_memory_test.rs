...
fn test_memory_reduction_very_large() {
    ...
    println!(
        "  Bitfield: {} bytes ({:.2} KB)",
        bf_memory,
        bf_memory as f64 / 1024.0
    );
    println!(
        "  Vec<bool>: {} bytes ({:.2} KB)",
        vec_memory,
        vec_memory as f64 / 1024.0
    );
    println!("  Ratio: {:.2}x", ratio);
    ...
    assert_eq!(
        bf_memory, 125_000,
        "1,000,000 bits should use 125,000 bytes"
    );
    assert_eq!(
        vec_memory, 1_000_000,
        "Vec<bool> should use 1,000,000 bytes"
    );
}
...
fn test_memory_savings_percentage() {
    ...
    for size in sizes {
        ...
        assert!(
            savings_pct >= 87.0,
            "Should achieve at least 87% memory savings"
        );
    }
...
fn test_real_world_scenario() {
    ...
    println!(
        "  Bitfield memory: {} bytes ({:.2} KB)",
        bf_memory,
        bf_memory as f64 / 1024.0
    );
    println!(
        "  Vec<bool> memory: {} bytes ({:.2} KB)",
        vec_memory,
        vec_memory as f64 / 1024.0
    );
    ...
    println!(
        ...
        (1.0 - bf_memory as f64 / vec_memory as f64) * 100.0
    );
    ...
    assert_eq!(
        bf.count_set(),
        16_667,
        "Should have 16,667 pieces downloaded"
    );
    assert!(ratio > 7.9, "Should achieve close to 8x memory savings");
}
